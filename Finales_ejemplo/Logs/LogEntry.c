...
int logEntry_filterLog(LogEntry* this)
{
    int retorno;

    if(logEntry_getGravedad(this) < 3)
    {
        retorno = 0;
    }
    if(logEntry_getGravedad(this) == 3 && logEntry_getServiceId(this) != 45)
    {
        retorno = 1;
    }
    if(logEntry_getGravedad(this) > 3 && logEntry_getGravedad(this) < 8)
    {
        retorno = 2;
    }
    if(logEntry_getGravedad(this)> 7)  // if (logEntry_getGravedad(item) > 7)
    {
        retorno = 3;
    }
    return retorno;
...
{
    printf("%s;%s;%d;%d;%s\n",  logEntry_getDate(this),logEntry_getTime(this),logEntry_getServiceId(this),
                                logEntry_getGravedad(this),logEntry_getMsg(this));
}

...
int val_validarEmail(char* buffer)
{
    ...
    while(buffer[i] != '\0')
    {
        if((buffer[i] < '0' || buffer[i] > '9') && (buffer[i] < 'a' || buffer[i] > 'z' ) && buffer[i] != 32 && buffer[i] != 64 && buffer[i] != 47 && buffer[i] != 22 &&(buffer[i] < 'A' || buffer[i] > 'Z' ))
        {
            retorno = -1;
            break;
        }
...
int val_validarCuit(char* buffer)
{
    ...
    while(buffer[i] != '\0')
    {
        if((buffer[i] < '0' || buffer[i] > '9' || strlen(buffer)>13 || strlen(buffer)<12 ) && buffer[i] != '-')
        {
            retorno = -1;
            break;
        }
...
int val_getTelefono(char* destino, char* mensaje, char* mensajeError, int intentos, int limite)
{
    ...
    if(destino != NULL && limite > 0 && limite < MAX_INPUT_BUFFER)
    {
        ...
        for( ; intentos>0; intentos--)
        {
            ...
            if(val_validarTelefono(buffer) == -1)
            {
                printf("%s",mensajeError);
                continue;
            }
